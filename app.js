...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function showMenuItems(items) {
    ...
    items.forEach((item) => {
      menuContainer.innerHTML += renderMenuItem(item); // Add each item to the container
    });
  }
  ...
  function filterMenuByCategory(category) {
    let filteredMenu;
    if (category === "all") {
      filteredMenu = menu; // Show all items if category is 'all'
    } else {
      filteredMenu = menu.filter((item) => item.category === category); // Filter items by category
    }
    showMenuItems(filteredMenu); // Display filtered items
  }

  // Function to filter menu items by search term
  function filterMenuBySearchTerm(searchTerm) {
  ...
  filterButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      filterMenuByCategory(category); // Filter items by category when button is clicked
    });
  });
  ...
  searchInput.addEventListener("input", function () {
    ...
    filterMenuBySearchTerm(searchTerm); // Filter items by search term as user types
  });
});
